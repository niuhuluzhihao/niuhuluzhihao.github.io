...
![1772077048369](/pict/2026-02-11-sft实践/1772077048369.png)
...
![1770948743148](/pict/2026-02-11-sft实践/1770948743148.png)

杂乱无章  ---batch太小了

![1770977165099](/pict/2026-02-11-sft实践/1770977165099.png)
...
[用FP8训练大模型有多香？微软：比BF16快64%，省42%内存 - 机器之心的文章 - 知乎](https://zhuanlan.zhihu.com/p/664744558)

[Claude AI Chat](https://claude.ai/chat/e72369ab-dd91-4cd3-946d-7e7e2abde717)
...
TYPE="zero3"
TRAIN_TYPE="single"
echo "每个节点启动的进程数:MLP_WORKER_GPU=$MLP_WORKER_GPU"
echo "主节点的ip地址:MLP_WORKER_0_HOST=$MLP_WORKER_NUM"
echo "当前节点的编号:MLP_ROLE_INDEX=$MLP_ROLE_INDEX"
echo "主节点的通信端口:MLP_WORKER_0_PORT=$MLP_WORKER_0_PORT"
echo "分布式训练的总节点数:MLP_WORKER_NUM=$MLP_WORKER_NUM"
CURRENT_TIME=$(date +"%Y%m%d_%H%M%S")
echo "CURRENT_TIME: $CURRENT_TIME"

export BASE_MODEL_NAME="Qwen3-32B"
export DOCKER_VEPFS="/vepfs/algorithm-csrobot"
export DATA_PATH="${DOCKER_VEPFS}/mzh/data/qwen3-30b-a3b/distill_r1_110k_sft_no_think.jsonl"
export MODEL_PATH="${DOCKER_VEPFS}/BaseModel/${BASE_MODEL_NAME}"
export SAVE_PATH="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/saves/${BASE_MODEL_NAME}/${TYPE}/train_${CURRENT_TIME}"
OUTPUT_FILE="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/logs/${BASE_MODEL_NAME}/${TYPE}/training_main_$(date +%Y%m%d_%H%M%S)_${TRAIN_TYPE}.log"

SWIFT_BIN="/usr/local/bin/swift"
cd "${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/" || {
  echo "Error: Failed to change directory to ${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/"
  exit 1
}

LOG_DIR="logs/${BASE_MODEL_NAME}"
MAIN_LOG_FILE=$(dirname "${OUTPUT_FILE}")
mkdir -p "${MAIN_LOG_FILE}"
mkdir -p "${LOG_DIR}"
mkdir -p "${SAVE_PATH}"

# 输出日志文件和参数信息文件
LOG_FILE="${LOG_DIR}/training_rank.log"
...
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export BASE_MODEL_NAME="Qwen3-30B-A3B-Base"
export TYPE='zero3'
export DOCKER_VEPFS="/vepfs/algorithm-csrobot"
export DATA_PATH="${DOCKER_VEPFS}/mzh/data/qwen3-30b-a3b/distill_r1_110k_sft_no_think.jsonl"
export MODEL_PATH="${DOCKER_VEPFS}/BaseModel/${BASE_MODEL_NAME}"
export SAVE_PATH="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/saves/${BASE_MODEL_NAME}/${TYPE}/train_${CURRENT_TIME}"
OUTPUT_FILE="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/logs/${BASE_MODEL_NAME}/${TYPE}/training_main_$(date +%Y%m%d_%H%M%S).log"

# SWIFT_BIN="/root/miniconda3/envs/ms-swift/bin/swift"      # 本地开发机
SWIFT_BIN="/usr/local/bin/swift"
cd "${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/" || {
  echo "Error: Failed to change directory to ${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/"
  exit 1
...
export NCCL_IB_DISABLE=0        # 禁用 / 启用 NCCL 使用 RDMA（IB 网卡）通信，0 表示启用，1 表示禁用。

echo "单机多卡的场景"
echo "每个节点启动的进程数:MLP_WORKER_GPU=$MLP_WORKER_GPU"
echo "主节点的ip地址:MLP_WORKER_0_HOST=$MLP_WORKER_0_HOST"
echo "当前节点的编号:MLP_ROLE_INDEX=$MLP_ROLE_INDEX"
echo "主节点的通信端口:MLP_WORKER_0_PORT=$MLP_WORKER_0_PORT"
echo "分布式训练的总节点数:MLP_WORKER_NUM=$MLP_WORKER_NUM"

echo "NCCL_DEBUG:NCCL_DEBUG=$NCCL_DEBUG"
echo "NCCL_IB_TIMEOUT:NCCL_IB_TIMEOUT=$NCCL_IB_TIMEOUT"
echo "NCCL_IB_RETRY_CNT:NCCL_IB_RETRY_CNT=$NCCL_IB_RETRY_CNT"
echo "NCCL_IB_DISABLE:NCCL_IB_DISABLE=$NCCL_IB_DISABLE"
echo "NCCL_IB_HCA:NCCL_IB_HCA=$NCCL_IB_HCA"
echo "NCCL_IB_GID_INDEX:NCCL_IB_GID_INDEX=$NCCL_IB_GID_INDEX"
echo "NCCL_SOCKET_IFNAME:NCCL_SOCKET_IFNAME=$NCCL_SOCKET_IFNAME"
echo "NCCL_IB_PCI_RELAXED_ORDERING:NCCL_IB_PCI_RELAXED_ORDERING=$NCCL_IB_PCI_RELAXED_ORDERING"
echo "NCCL_TOPO_FILE:NCCL_TOPO_FILE=$NCCL_TOPO_FILE"
echo "CURRENT_TIME: $CURRENT_TIME"
...
export MODEL_PATH="${DOCKER_VEPFS}/BaseModel/${BASE_MODEL_NAME}"
export SAVE_PATH="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/saves/${BASE_MODEL_NAME}/${TYPE}/train_${CURRENT_TIME}"
OUTPUT_FILE="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/logs/${BASE_MODEL_NAME}/${TYPE}/${ACCURACY}/training_main_$(date +%Y%m%d_%H%M%S)_${TRAIN_TYPE}.log"

SWIFT_BIN="/usr/local/bin/megatron"
cd "${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/" || {
  echo "Error: Failed to change directory to ${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/"
  exit 1
}
...
export DATA_PATH="${DOCKER_VEPFS}/mzh/data/qwen3-30b-a3b/distill_r1_110k_sft_no_think.jsonl"
export MODEL_PATH="${DOCKER_VEPFS}/BaseModel/${BASE_MODEL_NAME}"
export SAVE_PATH="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/saves/${BASE_MODEL_NAME}/${TYPE}/${ACCURACY}/train_${CURRENT_TIME}"
OUTPUT_FILE="${DOCKER_VEPFS}/mzh/ms-swift-release-3.12/logs/${BASE_MODEL_NAME}/${TYPE}/${ACCURACY}/training_main_$(date +%Y%m%d_%H%M%S)_${TRAIN_TYPE}.log"
