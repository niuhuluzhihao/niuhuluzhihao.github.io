...
低秩分解---是一种用于简化矩阵或张量结构的核心技术。它的核心思想是：发现大规模数据中隐藏的低维结构，将一个高秩的矩阵或张量近似表示为多个低秩矩阵或张量的组合。

矩阵的**秩** --**秩**是描述矩阵“本质特征”和“包含有效信息量”的最核心概念之一。简单来说，它衡量了矩阵中有多少行（或列）是真正独立、不重复的。
...
![1780991206467](/images/2026-05-07-大模型技术报告/1780991206467.png)

1. 概念来源：信息检索系统的“解耦”这个设计灵感来源于数据库或信息检索系统（比如你在搜索引擎里搜视频）：Query ($Q$)：你想找什么（比如输入搜索框的关键词：“猫咪 搞笑”）。Key ($K$)：视频的标签/索引（比如平台视频的标签：“宠物”、“搞笑”、“日常”）。Value ($V$)：视频的实际内容（点击进去看到的视频画面）。如果不用三个向量，而是直接拿输入向量 $X$ 自己跟自己算相似度：这意味着：一个词“希望别人怎么看待它”（作为被查询者）和“它想去寻找什么”（作为查询者），在数学上被完全绑定了。通过引入 $Q$ 和 $K$，我们将“主动寻找”和“被动匹配”两个角色分离开来。同一个词，在扮演 $Q$ 时可以去寻找动词，在扮演 $K$ 时可以被形容词修饰。通过引入 $V$，我们将“计算关联度的权重”与“实际传递的信息”分离开来。
...
![1780975610146](/images/2026-05-07-大模型技术报告/1780975610146.png)
